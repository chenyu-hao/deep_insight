...
基于 FastAPI 实现�?MCP 兼容服务器，
暴露舆论分析工具�?AI 助手调用�?

支持两种协议:
1. 标准 MCP 协议 (SSE + JSON-RPC) - 用于 ClawdBot �?MCP 客户�?
2. HTTP REST API - 用于直接调用和测�?
...
# 导入所有工具函�?
from app.opinion_mcp.tools import (
    analyze_topic,
    get_analysis_status,
    get_analysis_result,
    update_copywriting,
...
from app.opinion_mcp.tools.validate_publish import validate_publish
...
async def lifespan(app: FastAPI):
    ...
    yield
    logger.info("Opinion MCP Server 关闭�?..")


app = FastAPI(
    title="Opinion MCP Server",
    description="GlobalInSight 舆论分析 MCP 服务 - 提供多平台舆情分析、热榜获取、小红书发布等功�?,
    version="1.0.0",
    lifespan=lifespan,
)
...
MCP_TOOLS: List[MCPTool] = [
        ...
        inputSchema=MCPToolInput(
            ...
            properties={
                ...
                "platforms": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "要爬取的平台列表，可选�? wb, dy, ks, bili, tieba, zhihu, xhs, hn, reddit。留空则使用默认平台"
                },
                "debate_rounds": {
                    "type": "integer",
                    "minimum": 1,
                    "maximum": 5,
                    "default": 2,
                    "description": "辩论轮数 (1-5)，影响分析深度，默认2�?
                },
                "image_count": {
                    "type": "integer",
                    "minimum": 0,
                    "maximum": 9,
                    "default": 2,
                    "description": "生成图片数量 (0-9)�?表示不生图，默认2�?
                }
            },
            required=["topic"]
        )
    ...
    MCPTool(
        ...
        inputSchema=MCPToolInput(
            ...
            properties={
                "job_id": {
                    "type": "string",
                    "description": "任务 ID，由 analyze_topic 返回。留空则查询最近一次任�?
                }
            },
            required=[]
        )
    ...
    MCPTool(
        name="get_hot_news",
        description="获取多平台热榜数据，可用于发现热门话�?,
        inputSchema=MCPToolInput(
            type="object",
            properties={
                "limit": {
                    ...
                    "maximum": 100,
                    "default": 20,
                    "description": "返回条数 (1-100)，默�?20"
                },
                "platforms": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "筛选平台列表，留空返回所有平�?
                },
                "include_hn": {
                    "type": "boolean",
                    "default": True,
                    "description": "是否包含 Hacker News，默�?True"
                }
            },
            required=[]
        )
        ...
        inputSchema=MCPToolInput(
            ...
            properties={
                ...
                "tags": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "话题标签列表，留空则使用分析结果的标�?
                }
            },
            required=["job_id"]
        )
    ...
    MCPTool(
        ...
        description="验证发布条件是否满足。检�?XHS-MCP 服务状态、任务完成状态、图�?URL 有效性，返回详细验证结果和修复建议�?,
        inputSchema=MCPToolInput(
            type="object",
            properties={
                "job_id": {
    ...
    MCPTool(
        ...
        description="为已完成的分析任务生成数据卡片图片（标题卡、观点卡、辩论时间线、趋势图、雷达图、核心发现、平台热度）。需要先启动渲染服务 (frontend/renderer/)�?,
        inputSchema=MCPToolInput(
            type="object",
            properties={
                "job_id": {
                ...
                "card_types": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "指定要生成的卡片类型，留空默认生成四张核心卡（title, debate_timeline, trend, radar）。可�? title, insight, debate_timeline, trend, radar, key_findings, platform_heat"
                }
            },
            required=[]
        )
...
async def sse_endpoint(request: Request):
    ...
    async def event_generator():
        try:
            # 发�?endpoint 事件，告知客户端 POST 消息的地址
...
async def message_endpoint(request: Request, sessionId: str = None):
    ...
    客户端通过此端点发�?JSON-RPC 请求，响应通过 SSE 推送�?
    """
    try:
        body = await request.json()
    except Exception as e:
        logger.error(f"[MCP] 解析请求体失�? {e}")
...
async def root_post(request: Request):
    """根路�?POST - 处理 JSON-RPC 请求（兼�?mcporter�?""
    try:
        body = await request.json()
    except Exception as e:
        logger.error(f"[MCP] 解析请求体失�? {e}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
    python -m app.opinion_mcp.server                    # 使用默认端口 18061
    python -m app.opinion_mcp.server --port 18062      # 使用自定义端�?
    python -m app.opinion_mcp.server --host 0.0.0.0    # 监听所有接口
        """,
    )
    ...
    parser.add_argument(
        ...
        help=f"服务器端�?(默认: {config.MCP_PORT})",
    )
    ...
    parser.add_argument(
        ...
        help=f"服务器主�?(默认: {config.MCP_HOST})",
    )
...
def setup_signal_handlers() -> None:
    ...
    def signal_handler(signum, frame):
        ...
        logger.info(f"收到信号 {sig_name}，正在优雅关�?..")
...
def log_startup_info(host: str, port: int) -> None:
    ...
    logger.info(f"  健康检�? http://{host}:{port}/health")
...
def main() -> None:
    ...
    try:
        ...
        logger.info("收到键盘中断，正在关�?..")
    except Exception as e:
        logger.exception(f"服务器运行出�? {e}")
        sys.exit(1)
    finally:
        logger.info("服务器已关闭")
