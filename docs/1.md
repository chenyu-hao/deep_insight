...
4. **多媒体生成**：
    ...
    - 调用 [frontend/renderer/](file:///d:/python-project/GlobalInSight/frontend/renderer/) 渲染数据可视化卡片。
5. **成果产出与发布**：
    - [xiaohongshu_publisher.py](file:///d:/python-project/GlobalInSight/app/services/xiaohongshu_publisher.py) 调用 [external/XHS-MCP](file:///d:/python-project/GlobalInSight/external/XHS-MCP/) 驱动将报告发布至小红书。
...
| `external/MediaCrawler/config.py` | `ENABLE_IP_PROXY` | **反爬配置**：是否启用代理 IP 池。 |
| `external/MediaCrawler/config.py` | `CRAWLER_MAX_SLEEP_SEC` | **频率控制**：模拟真人操作的随机睡眠时间上限。 |
...
| `external/XHS-MCP/` | 主线二 | 最终成果发布到小红书的“执行官” |
